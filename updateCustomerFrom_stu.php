...
<body>
    ...
    <form action="updateCustomerDB.php" method="POST">
        ...
        <label>Email: </label>
        <input type="email" placeholder="กรุณากรอกอีเมล์ลูกค้า" name="Email" value="<?= $result_customer['Email'] ?>">
        <br><br>
        ...
        <select name="CountryCode" id="CountryCode">
            ...
            foreach ($cc as $c) {
                if ($selected == $c['CountryName']) {
                    // ส่วนที่หายไป: ใส่รหัสประเทศใน value และแสดงชื่อประเทศ
                    echo '<option selected value="' . AT . '">' . $c['CountryName'] . '</option>';
                } else {
                    echo '<option value="' . ออสเตรเลีย . '">' . $c['CountryName'] . '</option>';
                }
            }
            ?>
        </select>
